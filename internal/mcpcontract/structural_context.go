...
func WithStructuralContextV2(m *Manifest) *Manifest {
	...
	c.Tools = append(c.Tools, ToolContract{Name: StructuralContextV2Tool, InputSchemaID: StructuralContextV2InputID, EnvelopeSchemaIDs: []string{StructuralContextV2SuccessID, StructuralContextV2DomainErrorID}, ArtifactSchemaIDs: []string{StructuralContextV2ResultID}, Advertised: true, Availability: "ENABLED"})
	return &c
}
...
func ValidateStructuralContextV2EnvelopeExclusive(data []byte) error {
	...
	compiler := jsonschema.NewCompiler()
	compiler.DefaultDraft(jsonschema.Draft2020)
	for _, id := range []string{StructuralContextV2ResultID, StructuralContextV2SuccessID, StructuralContextV2DomainErrorID} {
		raw, _ := StructuralContextSchemaJSON(id)
		doc, e := jsonschema.UnmarshalJSON(bytes.NewReader(raw))
		if e != nil {
			return e
	...
	matches := []string{}
	for _, id := range []string{StructuralContextV2SuccessID, StructuralContextV2DomainErrorID} {
		s, e := compiler.Compile(id)
		if e != nil {
			return e
		}
