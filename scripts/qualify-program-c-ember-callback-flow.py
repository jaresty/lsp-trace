#!/usr/bin/env python3
"""Private, tuple-specific Program C Ember callback-flow qualification."""
import argparse, hashlib, json, subprocess, sys
from pathlib import Path

PROFILE = "callback-flow-v1"
COORDINATE = ("typescript", "ember", "ember-glint", "1.0.3")
RELATIONS = ("BINDS_ARGUMENT", "PASSES_CALLBACK", "INVOKES_TASK")
SCHEMA = "lsp-trace.program-c-profile-qualification-receipt.v1"
TOOL_VERSION = "program-c-ember-callback-flow-qualifier/1"
...
def assert_tool_revision(root, revision):
    path = "scripts/qualify-program-c-ember-callback-flow.py"
...
def run_provider(provider, fixture, relation, revision, generation):
    ...
    request = {"schema":"lsp-trace.provider-request.v1","request_id":f"program-c:{generation}:{relation}","operation":"analyze","relation_kinds":[relation],"documents":[{"uri":fixture.resolve().as_uri(),"language":"glimmer-js","revision":revision,"digest":digest(source),"source":source.decode()}],"limits":{"max_observations":10000,"timeout_ms":30000}}
    ...
    if response.get("outcome") not in ("COMPLETE", "BOUNDED") or not observations:
...
def build(root, revision, matrix_bytes):
    ...
    fixtures = {
      "BINDS_ARGUMENT": root / "qualification/external-provider/component.gts",
      "PASSES_CALLBACK": root / "qualification/external-provider/passes-callback-positive.gts",
      "INVOKES_TASK": root / "providers/ember-glint/fixtures/source-constrained-synthetic/invokes-task-positive.js",
    }
    ...
    for relation in RELATIONS:
        ...
        if envelope.get("provider", {}).get("identity") != "ember-glint@1": raise ValueError(f"{relation}: observed provider identity mismatch")
...
def main():
    ...
    print(f"ASSERT_PROGRAM_C_EMBER_CALLBACK_FLOW_QUALIFICATION result=PASS counts="+",".join(f"{e['relation']}:{e['count']}" for e in receipt['relation_evidence']))
