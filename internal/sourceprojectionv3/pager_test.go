...
func testLimits() Limits {
	return Limits{MaxPageBytes: 150, MaxPages: 4, MaxResponseBytes: 1000, MaxObjects: 10, MaxRanges: 10, MaxSourceBytes: 100, MaxWork: 10}
}
...
func TestPaginateDeterministicReplayAndCumulativeAccounting(t *testing.T) {
	records := testRecords()
	limits := testLimits()
	first, err := Paginate(records, Request{Binding: testBinding(), Limits: limits})
	...
	replay, err := Paginate(records, Request{Binding: testBinding(), Limits: limits})
	if err != nil || !reflect.DeepEqual(first, replay) {
		t.Fatalf("ASSERT_V3_FIRST_PAGE_DETERMINISTIC: replay=%+v err=%v", replay, err)
	}
	second, err := Paginate(records, Request{Binding: testBinding(), Limits: limits, Cursor: first.NextCursor})
	...
	third, err := Paginate(records, Request{Binding: testBinding(), Limits: limits, Cursor: second.NextCursor})
...
func TestPaginateCursorBindingReplayAndTamperFailClosed(t *testing.T) {
	first, err := Paginate(testRecords(), Request{Binding: testBinding(), Limits: testLimits()})
	if err != nil {
		t.Fatal(err)
	}
	changed := testBinding()
	changed.CustodyMode = "RETAINED"
	if _, err := Paginate(testRecords(), Request{Binding: changed, Limits: testLimits(), Cursor: first.NextCursor}); !errors.Is(err, ErrInvalidCursor) {
	...
	if _, err := Paginate(testRecords(), Request{Binding: testBinding(), Limits: testLimits(), Cursor: string(tampered)}); !errors.Is(err, ErrInvalidCursor) {
...
func TestPaginateZeroAndCumulativeBoundsAreReal(t *testing.T) {
	limits := testLimits()
	limits.MaxObjects = 0
	if _, err := Paginate(testRecords(), Request{Binding: testBinding(), Limits: limits}); !errors.Is(err, ErrResourceLimit) {
		t.Fatalf("ASSERT_V3_ZERO_OBJECT_BOUND: %v", err)
	}
	limits = testLimits()
	limits.MaxPages = 1
	first, err := Paginate(testRecords(), Request{Binding: testBinding(), Limits: limits})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := Paginate(testRecords(), Request{Binding: testBinding(), Limits: limits, Cursor: first.NextCursor}); !errors.Is(err, ErrResourceLimit) {
		t.Fatalf("ASSERT_V3_PAGE_BOUND_CUMULATIVE: %v", err)
	}
	limits = testLimits()
	limits.MaxSourceBytes = 6
	if _, err := Paginate(testRecords(), Request{Binding: testBinding(), Limits: limits}); err != nil {
		t.Fatalf("first record should fit: %v", err)
	}
	first, _ = Paginate(testRecords(), Request{Binding: testBinding(), Limits: limits})
	if _, err := Paginate(testRecords(), Request{Binding: testBinding(), Limits: limits, Cursor: first.NextCursor}); !errors.Is(err, ErrResourceLimit) {
...
func TestPaginateRejectsOversizeDuplicateInvalidAndOverflowRecords(t *testing.T) {
	...
	if _, err := Paginate(records, Request{Binding: testBinding(), Limits: limits}); !errors.Is(err, ErrResourceLimit) {
		t.Fatalf("ASSERT_V3_WHOLE_RECORD_ATOMIC: %v", err)
	}
	records = testRecords()
	records[1].Key = records[0].Key
	if _, err := Paginate(records, Request{Binding: testBinding(), Limits: limits}); !errors.Is(err, ErrInvalidRequest) {
	...
	if _, err := Paginate(records, Request{Binding: testBinding(), Limits: limits, Cursor: first.NextCursor}); !errors.Is(err, ErrResourceLimit) {
