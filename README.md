...
Its schema is `lsp-trace.vcs-symbol-churn-sidecar.v2` (`lsp-trace schema get --family vcs-symbol-churn-sidecar --version v2`). MCP operation 39, `lsp_trace_v1_context_symbol_churn`, exposes the same builder using a host-configured language-server profile; callers cannot provide executable paths, arguments, environments, working directories, publication selectors, or retained inputs. Every changed-line observation is `ATTRIBUTED`, `AMBIGUOUS`, or `UNMATCHED`; every requested file has a `COMPLETE`, `EMPTY`, or `FAILED` acquisition row on each revision. The result retains `authority=0`, `source_graph_complete=UNKNOWN`, and `cross_revision_identity=NOT_EVALUATED`. Names and ranges do not establish symbol continuity, renames, ownership, defects, features, runtime behavior, or `CALLS`. V1 and MCP operation 38 remain unchanged.
