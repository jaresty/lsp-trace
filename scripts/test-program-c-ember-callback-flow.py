...
SCRIPT=ROOT/'scripts/qualify-program-c-ember-callback-flow.py'
...
def prepublication_matrix():
 ...
 for line in text.splitlines():
  if line.startswith('callback-flow-v1\ttypescript\tember\tember-glint\t1.0.3\t'):
...
class QualificationTest(unittest.TestCase):
 def test_positive_deterministic_strict_receipt(self):
  ...
  self.assertEqual(a,b,'ASSERT_PROGRAM_C_EMBER_CALLBACK_FLOW_DETERMINISTIC')
  self.assertEqual([e['relation'] for e in a['relation_evidence']],list(q.RELATIONS),'ASSERT_PROGRAM_C_EMBER_CALLBACK_FLOW_EXACT_RELATIONS')
  self.assertTrue(all(e['count']>0 and e['custody']=='PROVIDER_PROVED' and e['replay']=='EXACT_BYTES' for e in a['relation_evidence']),'ASSERT_PROGRAM_C_EMBER_CALLBACK_FLOW_PROVIDER_PROVED_REPLAY')
  print('ASSERT_PROGRAM_C_EMBER_CALLBACK_FLOW_RECEIPT result=PASS')
 def test_fail_closed_tampered_prepublication_matrix(self):
  ...
  print('ASSERT_PROGRAM_C_EMBER_CALLBACK_FLOW_PREPUBLICATION result=FAIL expected=true')
 def test_fail_closed_wrong_revision(self):
  ...
  self.assertNotEqual(result.returncode,0,'ASSERT_PROGRAM_C_EMBER_CALLBACK_FLOW_REVISION_FAIL_CLOSED')
  print('ASSERT_PROGRAM_C_EMBER_CALLBACK_FLOW_REVISION result=FAIL expected=true')
