...
func WithContextSymbolChurn(m *Manifest) *Manifest {
	...
	c.Tools = append(c.Tools, ToolContract{Name: ContextSymbolChurnTool, InputSchemaID: ContextSymbolChurnInputID, EnvelopeSchemaIDs: []string{ContextSymbolChurnSuccessID, ContextSymbolChurnDomainErrorID}, ArtifactSchemaIDs: []string{ContextSymbolChurnResultID}, Advertised: true, Availability: "ENABLED"})
	return &c
}

//go:embed testdata/schemas/*context-symbol-churn*.json testdata/schemas/lsp-trace.vcs-symbol-churn-sidecar.v2.schema.json
...
func ValidateContextSymbolChurnEnvelopeExclusive(raw []byte) error {
	...
	if id != ContextSymbolChurnSuccessID && id != ContextSymbolChurnDomainErrorID {
...
func contextSymbolChurnSchema(name string) ([]byte, bool, error) {
	paths := map[string]string{ContextSymbolChurnInputID: "testdata/schemas/input-context-symbol-churn.v1.schema.json", ContextSymbolChurnResultID: "testdata/schemas/lsp-trace.vcs-symbol-churn-sidecar.v2.schema.json", ContextSymbolChurnSuccessID: "testdata/schemas/envelope-context-symbol-churn-result.v1.schema.json", ContextSymbolChurnDomainErrorID: "testdata/schemas/envelope-context-symbol-churn-domain-error.v1.schema.json"}
