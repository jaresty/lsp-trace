...
func TestHostSelectorCompositionIncludesLifecycle(t *testing.T) {
	...
	if got := server.Registry.Tools(); len(got) != 39 {
...
func TestAlwaysLocalTraversalManagedFakeLSPEndToEnd(t *testing.T) {
	...
	if err := json.Unmarshal([]byte(lines[0]), &listed); err != nil || len(listed.Result.Tools) != 39 {
	...
	for i := 1; i < len(listed.Result.Tools); i++ {
		if listed.Result.Tools[i-1].Name > listed.Result.Tools[i].Name {
			t.Fatalf("ASSERT_ALWAYS_LOCAL_THIRTY_SEVEN_TOOL_ORDER: tools=%v", listed.Result.Tools)
		}
	}
...
func TestDefaultAndAdvancedProcessAdvertisement(t *testing.T) {
	want := map[string][]string{
		"default": {
			"lsp_trace_v1_capabilities", "lsp_trace_v1_census", "lsp_trace_v1_context_churn", "lsp_trace_v1_context_symbol_churn", "lsp_trace_v1_execute", "lsp_trace_v1_inspect_hydrated",
		...
		"advanced": {
			...
			"lsp_trace_v1_capabilities", "lsp_trace_v1_census", "lsp_trace_v1_context_churn", "lsp_trace_v1_context_symbol_churn", "lsp_trace_v1_custody_execute", "lsp_trace_v1_execute", "lsp_trace_v1_export_retained_calls",
