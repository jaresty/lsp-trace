...
Before grouping, perform an independent entry-point census rather than deriving the census from the captured graph. Acquire source-bearing evidence first; if required source bytes are unavailable, preserve that gap instead of treating structure as semantics. Keep the native directed graph intact and retain per-seed attribution rather than symmetrizing edges, flattening direction, or assigning the union graph to each seed.
