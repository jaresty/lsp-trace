...
`elixir-companion` is an opt-in, local-only qualification mode. It builds and invokes the separately versioned [jaresty/elixir-call-hierarchy](https://github.com/jaresty/elixir-call-hierarchy) escript and writes evidence under `qualification/evidence/elixir-companion/`; it does not replace or rewrite the retained ElixirLS BLOCKED evidence under `qualification/retained/elixir/`. By default the script uses a sibling `../elixir-call-hierarchy` checkout; set `ELIXIR_CALL_HIERARCHY_ROOT` to another checkout path. The external server's own README defines its profiling, trust, completeness, and clause-coalescing contracts. The companion assertion report covers same-module, recursive, static-only, aliased cross-file, fully qualified cross-file, multi-clause target resolution, and exact non-empty ranges.
