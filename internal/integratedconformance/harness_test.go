	...
	t.Run("ASSERT_PACKAGE_OWNERSHIP_ONLY", func(t *testing.T) {
		...
		for _, line := range strings.Split(strings.TrimSpace(string(out)), "\n") {
			...
			owned := path == "sessionruntime/sessionruntime.go" || path == "sessionruntime/sessionruntime_test.go" || path == "sessionruntime/roundtrip_test.go" || path == "sessionruntime/local_darwin_integration_test.go" ||
				...
				path == "docs/adr/0004-analysis-only-type-overlay-qualification.md" || path == "qualification/README.md" || strings.HasPrefix(path, "qualification/type-overlay/") || path == "scripts/qualify-b05-type-overlay.sh" || path == "internal/schema/type_overlay_qualification_test.go" || path == "internal/schema/schemas/lsp-trace.analysis-only-type-overlay-qualification.v1.schema.json" || path == "schema/schemas/lsp-trace.analysis-only-type-overlay-qualification.v1.schema.json"
			if !owned {
