	...
	t.Run("ASSERT_PACKAGE_OWNERSHIP_ONLY", func(t *testing.T) {
		...
		for _, line := range strings.Split(strings.TrimSpace(string(out)), "\n") {
			...
			owned := path == "sessionruntime/sessionruntime.go" || path == "sessionruntime/sessionruntime_test.go" || path == "sessionruntime/roundtrip_test.go" || path == "sessionruntime/local_darwin_integration_test.go" ||
				...
				path == "cmd/lsp-trace/main.go" || path == "cmd/lsp-trace/main_test.go" || path == "cmd/lsp-trace/slice_command.go" || path == "fr1-typescript-call-hierarchy.claim.md" || path == "cmd/lsp-trace/execution_command.go" || path == "cmd/lsp-trace/execution_command_test.go" || path == "cmd/lsp-trace/render_command.go" || path == "cmd/lsp-trace/render_command_test.go" || path == "cmd/lsp-trace/custody_command.go" || path == "cmd/lsp-trace/custody_command_test.go" || strings.HasPrefix(path, "internal/presentation/") ||
