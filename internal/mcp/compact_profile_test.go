...
func TestToolProfilesPreserveFullAndCompactAdvertisement(t *testing.T) {
	for name, registry := range map[string]*Registry{
		...
		if got := len(registry.Tools()); got != 41 {
			t.Fatalf("ASSERT_%s_DISPATCHABLE_41: got %d", name, got)
		}
		if got := len(registry.Advertised()); got != 41 {
			t.Fatalf("ASSERT_%s_ADVERTISED_41: got %d", name, got)
		}
	...
	if got := len(compact.Tools()); got != 41 {
		t.Fatalf("ASSERT_COMPACT_DISPATCHABLE_41: got %d", got)
	}
...
func TestDescribeHiddenOperationFromCompactRegistry(t *testing.T) {
	...
	if len(r.Advertised()) != 12 || len(r.Tools()) != 41 {
...
func TestCompactCapabilitiesAreExactAndDescriptionsSelfContained(t *testing.T) {
	...
	if got := caps["dispatchable_tool_names"].([]string); len(got) != 41 {
		t.Fatalf("ASSERT_CAPABILITY_DISPATCHABLE_41: %v", got)
	}
