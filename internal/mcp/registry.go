...
func newRegistryWithRoutingAndProfile(publicationSupported bool, routing Routing, profile ToolProfile) *Registry {
	...
	descriptions := map[string]string{
		...
		mcpcontract.StructuralContextV2Tool:       "Analyze an exact symbol, position, or bounded document regex match in one READY managed session. Symbol and regex lookup are locator-only; CALLS remain server-reported. Optional source projection is bounded. Authority is zero and source completeness is unknown.",
		...
		"lsp_trace_v1_incoming":                   "Prefer this to answer who calls an exact callee: it directly observes exact caller relationships from server call hierarchy. Textual occurrences do not establish calls. Requires a READY managed session and exact document URI.",
		"lsp_trace_v1_slice":                      "Prefer this for an exact target's caller/callee neighborhood from server call hierarchy. Textual occurrences do not establish calls. Requires a READY managed session and exact document URI.",
	}
