...
func TestLifecycleExecutorFamilyIsEnabledAndAdvertisedByDefault(t *testing.T) {
	...
	t.Log("ASSERTION: " + assertion)
	r := NewRegistry(false)
	if got := len(r.Advertised()); got != 16 {
	...
	want := []string{"lsp_session_v1_list", "lsp_session_v1_restart", "lsp_session_v1_status", "lsp_session_v1_stop", "lsp_trace_v1_bounded_retained_analysis", "lsp_trace_v1_bounded_retained_metrics", "lsp_trace_v1_capabilities", "lsp_trace_v1_execute", "lsp_trace_v1_export_retained_calls", "lsp_trace_v1_filter", "lsp_trace_v1_incoming", "lsp_trace_v1_inspect", "lsp_trace_v1_schema_get", "lsp_trace_v1_slice", "lsp_trace_v1_validate", "lsp_trace_v1_verify"}
...
func TestTraversalSchemaSelectorContracts(t *testing.T) {
	...
	if _, ok := properties["symbol"]; !ok || tool.InputSchema["oneOf"] == nil || len(NewRegistry(false).Advertised()) != 16 {
