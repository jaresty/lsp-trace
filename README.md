...
`--print-bootstrap-example` prints the exact safe template committed at [`cmd/lsp-trace-mcp/bootstrap.example.v1.json`](cmd/lsp-trace-mcp/bootstrap.example.v1.json) and exits without starting a process. The template shows both `gopls` and `csharp-ls`, including ordered `execution.arguments`. Replace only the explicit placeholders with independently trusted local values; absolute values are also accepted. The template is not an active configuration: pass the chosen file with `--bootstrap-config`, and restart the owning `lsp-trace-mcp` process after changing it because bootstrap is read only at startup.
