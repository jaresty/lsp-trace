...
Operation 31 is `lsp_trace_v1_program_c_compose`; CLI parity is `lsp-trace program-c compose PATH|-`. It deterministically composes 2–16 compatible Graph Provenance V5 captures, retaining each exact constituent byte string, digest, length, and identity. It unions only native constituent nodes and CALLS records, never infers cross-capture CALLS, and labels the result as a multi-capture composite rather than native single-capture custody. Incompatible coordinates, conflicting identities, invalid requested input schemas, and non-replayable composites fail closed. The compact profile remains exactly ten direct tools; operation 31 stays hidden-but-dispatchable through canonical execute.
