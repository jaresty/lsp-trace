...
func TestCompactResponsePublishesFullArtifactWithUsabilityMetadata(t *testing.T) {
	...
	if got := len(server.Registry.Advertised()); got != 30 {
...
func TestTraversalCompactResponsePublishesFullArtifact(t *testing.T) {
	const (
		...
		cardinality = "ASSERT_TRAVERSAL_COMPACT_PRESERVES_THIRTY_TOOLS"
	)
...
func TestRealTraversalEnvelopesValidateAcrossSuccessFailureAndPublication(t *testing.T) {
	const (
		...
		cardinalityAssertion = "ASSERT_REAL_TRAVERSAL_CURRENT30_PRESERVES_HISTORICAL_TOOLS"
	)
...
func TestCapabilitiesDispatch(t *testing.T) {
	const capabilityAssertion = "capabilities returns immutable metadata for all 30 current canonical tools"
...
func TestTransportContract(t *testing.T) {
	...
	const listAssertion = "tools/list advertises the 30 current canonical names"
