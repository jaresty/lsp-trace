...
func traceInputSchema() map[string]any {
	...
	props := map[string]any{"session_id": text(), "generation": integer(1, ^uint64(0)), "uri": text("uri"), "symbol": text(), "positions": map[string]any{"type": "array", "minItems": 1, "maxItems": 64, "items": position}, "language_id": text(), "down_depth": integer(1, 64), "up_depth": integer(1, 64), "max_nodes": integer(1, 10000), "timeout_ms": integer(1, 60000), "request_timeout_ms": integer(1, 60000), "topmost_siblings": map[string]any{"type": "boolean"}, "detail": map[string]any{"type": "string", "enum": []any{"compact", "full"}}, "output_selector": text()}
