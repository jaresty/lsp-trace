	...
	t.Run("ASSERT_PACKAGE_OWNERSHIP_ONLY", func(t *testing.T) {
		...
		for _, line := range strings.Split(strings.TrimSpace(string(out)), "\n") {
			...
			owned := path == "sessionruntime/sessionruntime.go" || path == "sessionruntime/sessionruntime_test.go" || path == "sessionruntime/roundtrip_test.go" || path == "sessionruntime/local_darwin_integration_test.go" ||
				...
				path == "caller-mcp-ledger.md" || path == "docs/PROVIDERS.md" || path == "docs/RELEASING.md" || path == "scripts/qualify-caller-project-javascript.sh" || path == "scripts/test-caller-project-javascript-qualification.sh" || strings.HasPrefix(path, "qualification/caller-project-javascript/") || strings.HasPrefix(path, "qualification/retained/caller-project-javascript/")
			if !owned {
