...
When a task asks for a semantic relationship that LSP Trace directly observes—especially callers, callees, execution paths, call dependency neighborhoods, or incoming/outgoing impact—prefer bounded semantic structural tools before textual occurrences. Text search is appropriate for lexical facts such as schema IDs, constants, configuration, manifests, and profile membership, or only as a locator fallback when semantic resolution honestly fails. Textual occurrences never establish CALLS. After structural tracing, read source bodies for semantic interpretation.
