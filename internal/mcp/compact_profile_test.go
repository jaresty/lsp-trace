...
func TestToolProfilesPreserveFullAndCompactAdvertisement(t *testing.T) {
	for name, registry := range map[string]*Registry{
		...
		if got := len(registry.Tools()); got != 31 {
			t.Fatalf("ASSERT_%s_DISPATCHABLE_31: got %d", name, got)
		}
		if got := len(registry.Advertised()); got != 31 {
			t.Fatalf("ASSERT_%s_ADVERTISED_31: got %d", name, got)
		}
	...
	if got := len(compact.Tools()); got != 31 {
		t.Fatalf("ASSERT_COMPACT_DISPATCHABLE_31: got %d", got)
	}
...
func TestCompactCapabilitiesAreExactAndDescriptionsSelfContained(t *testing.T) {
	...
	if got := caps["dispatchable_tool_names"].([]string); len(got) != 31 {
		t.Fatalf("ASSERT_CAPABILITY_DISPATCHABLE_31: %v", got)
	}
