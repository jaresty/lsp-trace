...
V3 may include an optional `slice` object for the `slice` command. It records the source URI, directional depth bounds, shortest-distance outgoing layers, exact frontier node IDs, and outgoing relation IDs. Every referenced node and relation must exist in the same native graph, layers must be contiguous from zero, and the frontier must equal the requested downward layer when that layer exists. If all branches end before that depth, the frontier is empty. V1 and v2 projections never include slice metadata.
