...
The final-name creation is the namespace commit point. An existing final bundle returns `TARGET_EXISTS` and is never changed. Concurrent same-selector publication has exactly one winner. After commit, the implementation opens the final descriptor-relative object and verifies regular-file identity, owner-only mode, length, exact canonical bytes, digest, native V5 admission, and manifest bijection. A late verification problem is represented as committed success with verification status, never as a failure that suggests visibility was rolled back. Directory sync records only that the final directory was sync-requested; it is not a whole-filesystem, device, or crash-recovery guarantee.
...
The returned `PublicationReceipt` is `PRIVATE` by default. It records selector, exact bundle SHA-256, bundle byte length, constituent count, namespace atomicity, committed verification status, and `exact_fd_atomic_no_replace`; it is not a public receipt schema and conveys no additional authority.
