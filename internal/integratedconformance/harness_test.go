	...
	t.Run("ASSERT_PACKAGE_OWNERSHIP_ONLY", func(t *testing.T) {
		...
		for _, line := range strings.Split(strings.TrimSpace(string(out)), "\n") {
			...
			owned := ownsFR20FR21Path(path) || path == "sessionruntime/sessionruntime.go" || path == "sessionruntime/sessionruntime_test.go" || path == "sessionruntime/roundtrip_test.go" || path == "sessionruntime/local_darwin_integration_test.go" ||
				...
				path == "cmd/lsp-trace/retained_calls.go" || path == "cmd/lsp-trace/inspect_command.go" || path == "cmd/lsp-trace/inspect_hydrated.go" || path == "cmd/lsp-trace/inspect_hydrated_test.go" || path == "cmd/lsp-trace-mcp/hydrated_public_test.go" || strings.HasPrefix(path, "internal/operation/inspect_hydrated") || path == "docs/retained-calls.md" || path == "docs/hydrated-evidence.md" ||
