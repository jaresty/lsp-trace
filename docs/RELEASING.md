...
`./scripts/release-check.sh` is the hermetic release dry-run. It validates required documentation, fixtures, normalized retained PASS evidence, GoReleaser v2 configuration, and the embedded Stage 1 MCP manifest/schema contract. It runs the focused MCP contract suite, then uses `go build -trimpath` to require nonempty `lsp-trace` and `lsp-trace-mcp` binaries in a temporary directory. It does not publish, create a tag, modify retained evidence, or invoke GoReleaser. It does not start or install external language servers. CI runs this dry-run plus formatting, Python/shell syntax, `go test ./...`, `go vet ./...`, `go build ./...`, and clean-tree checks.
