...
func TestLifecycleExecutorFamilyIsEnabledAndAdvertisedByDefault(t *testing.T) {
	const assertion = "ASSERT_ALWAYS_LOCAL_THIRTY_EIGHT_TOOL_ORDER"
	t.Log("ASSERTION: " + assertion)
	r := NewRegistry(false)
	if got := len(r.Advertised()); got != 43 {
...
func TestSemanticTraversalDescriptionsRouteAndExplainInvocation(t *testing.T) {
	...
	for name, phrases := range checks {
		...
		if strings.Contains(strings.ToLower(tool.Description), "nn ") {
			t.Errorf("ASSERT_SEMANTIC_ROUTING_DESCRIPTION[%s]: competing product named", name)
		}
	}
}
...
func TestTraversalSchemaSelectorContracts(t *testing.T) {
	...
	if _, ok := properties["symbol"]; !ok || tool.InputSchema["oneOf"] == nil || len(NewRegistry(false).Advertised()) != 43 {
...
func TestVerifyAdvertisesGraphProvenanceV5WithoutChangingCardinality(t *testing.T) {
	...
	if got := len(registry.Tools()); got != 43 {
		t.Fatalf("ASSERT_VERIFY_V5_CARDINALITY: got=%d want=35", got)
	}
...
func TestCanonicalDescriptionsRouteUserIntent(t *testing.T) {
	...
	want := map[string][]string{
		...
		"lsp_trace_v1_capabilities":          {"canonical", "limits"},
		"lsp_trace_v1_export_retained_calls": {"retained", "offline"},
		"lsp_trace_v2_export_retained_calls": {"retained-calls/v2", "not deployed"},
		"lsp_trace_v2_verify_retained_calls": {"retained-calls/v2", "custody"},
		"lsp_trace_v1_execute":               {"execute", "request"},
	}
	registry := NewRegistry(false)
