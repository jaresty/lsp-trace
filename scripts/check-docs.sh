...
assert_contains DOC-PROFILE-INDEPENDENCE cmd/lsp-trace/SKILL.md 'it is unrelated to the CLI `--profile NAME` server launch profile and changes neither dispatch nor managed-session behavior.'
...
assert_contains DOC-MCP-FULL-COMPAT README.md 'compatibility default `--tool-profile full` publishes exactly 28 canonical tools'
...
assert_contains DOC-PI-COMPACT-TEN README.md 'The recommended compact tool-advertisement profile advertises exactly ten canonical MCP names'
