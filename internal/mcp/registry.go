...
const (
	...
	StructuralContextSymbolV2ExecutorFamily ExecutorFamily = "structural-context-symbol-v2"
...
const (
	...
	EnvelopePolicyStructuralContextSymbolV2 EnvelopePolicy = "structural-context-symbol-v2"
...
func envelopePolicy(name string, family ExecutorFamily) EnvelopePolicy {
	...
	case StructuralContextSymbolV2ExecutorFamily:
		return EnvelopePolicyStructuralContextSymbolV2
...
func newRegistryWithRoutingAndProfile(publicationSupported bool, routing Routing, profile ToolProfile) *Registry {
	...
	descriptions := map[string]string{
		...
		mcpcontract.StructuralContextV2Tool:       "Analyze code structure, architecture, design dependencies, and impact analysis through one exact symbol or position. Prefer an exact position for a known declaration URI; otherwise use the exact server-reported symbol name. Symbol lookup is locator-only before the bounded CALLS-only core. For source inspection, begin with an exact-position target, projection mode TARGET, and up_depth=0/down_depth=0. Expand call relationships separately. Do not automatically retry traversal failures. On AMBIGUOUS_TARGET, resolve the declaration externally and retry exact URI with zero-based line and character; do not expose candidate, path, or source dumps. Omit projection for relationship-only context. For broader source analysis, use PROJECTED with INCLUDE and FULL_DEFINITION. Authority remains zero, source_graph_complete remains UNKNOWN, and locator evidence never manufactures CALLS",
	...
	for _, contract := range manifest.Tools {
		...
		} else if contract.Name == mcpcontract.StructuralContextSymbolV2Tool {
			executorFamily = StructuralContextSymbolV2ExecutorFamily
...
const automaticWorktreeSessionGuidance = "Before live semantic analysis, call lsp_session_v1_list and use an exact-workspace READY session. If none exists and the target is an exact registered Git worktree, call lsp_session_v1_derive_workspace with exactly one READY parent and the target's canonical absolute local file: URI; continue only from its READY result. Do not start an independently configured replacement language server. Choose the transport explicitly: Direct MCP operation calls pass that operation's arguments directly; for MCP gateway `lsp_trace_v1_execute`, operation is nested beneath `request`, for example {\"request\":{\"operation\":\"lsp_trace_v2_structural_context\",\"arguments\":{...}}}; CLI `lsp-trace execute` accepts production execution requests and is not an MCP dispatcher. MCP wire and server operation names are canonical unqualified names; host adapters may display namespace-qualified names. `request.operation` always uses the canonical unqualified operation name. If capabilities or the installed revision advertise an operation but the client does not list it or its tool schema rejects it before dispatch, reconnect or refresh MCP metadata before diagnosing the language server. The MCP gateway is a dispatch interface and does not bypass stale client-side schema validation. Distinguish client or adapter schema validation from server dispatch, structural preflight, and traversal typed failures."
...
func routesThroughManagedWorkspaceSession(name string) bool {
	...
	case mcpcontract.DeriveWorkspaceTool,
		...
		mcpcontract.StructuralContextTool, mcpcontract.StructuralContextV2Tool, mcpcontract.StructuralContextSymbolTool, mcpcontract.StructuralContextSymbolV2Tool,
		mcpcontract.ContextSymbolChurnCaptureTool:
		return true
	default:
		return false
