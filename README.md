...
The `--tool-profile full` profile publishes exactly 41 canonical tools. For normal MCP clients, prefer `lsp-trace-mcp --tool-profile compact`: it advertises twelve curated lifecycle, capability, gateway, incoming, hydrated-inspection, schema, slice, and unified structural-context tools. Hidden canonical operations remain reachable through `lsp_trace_v1_execute`; the removed legacy symbol-operation names do not resolve through either transport. Here, **MCP tool-advertisement profile** means only the set returned by `tools/list`; it does not change dispatch, operation behavior, authority, or managed sessions, and it is unrelated to the CLI's `--profile NAME` server launch configuration.
...
`lsp_trace_v2_structural_context` accepts exactly one target: either a known exact symbol whose document URI is unknown, or an exact URI/line/character position. Symbol targets resolve exactly one confined server-reported workspace-symbol document as locator-only control flow before delegating to the same source-bearing Structural Context V2 core used by position targets. The operation is advertised in compact discovery, callable directly or through `lsp_trace_v1_execute`, preserves authority `0` and source-graph completeness `UNKNOWN`, and never infers `CALLS` from text or workspace-symbol evidence.

For multiple exact symbols, call `lsp_trace_v2_structural_context` independently once per symbol and retain every typed envelope, including absent, ambiguous, partial, truncated, or failed results. Do not combine symbol names into a textual search: text can locate declarations or tests, but it cannot establish `CALLS`.
...
The recommended compact tool-advertisement profile advertises exactly twelve canonical MCP names. Hidden canonical tools remain dispatchable through the gateway, but removed legacy operation names are not. The additive `lsp_trace_v1_export_retained_calls` remains available through MCP discovery/proxy unless explicitly added to a client whitelist. `toolPrefix: "none"` keeps names unchanged in Pi. `searchKeywords` is adapter-only routing metadata: it improves proxy search without changing MCP names, descriptions, schemas, direct-tool registration, runtime behavior, or authority. Do not add a repository-local Pi extension or a second MCP bridge. Only the host-authored `.mcp.json` command, arguments, and bootstrap file choose executable, environment, or working directory. MCP callers cannot override process configuration.
