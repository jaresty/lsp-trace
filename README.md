...
- `--output PATH`: privately publish JSON. On POSIX, files use mode `0600` and v3 generation directories use `0700`; on Windows, privacy follows native account access controls and no POSIX mode-bit guarantee is asserted. V3 makes `PATH` an atomic selector for an immutable generation containing the graph and receipt.
