...
def main():
 ...
 c=[('ASSERT_I04_SCHEMA',d.get('schema_version')=='lsp-trace.program-c-license-inputs.v1'),('ASSERT_I04_GONUM_BSD',all(x in t for x in ('Gonum v0.17.0','BSD-3-Clause','native-Go linked module','blob/v0.17.0/LICENSE'))),('ASSERT_I04_INFOMAP_GPL_EXTERNAL',all(x in t for x in ('Infomap v2.15.1','GPL-3.0-or-later','LICENSE_GPLv3.txt','externally provisioned CLI/reference only','never linked or bundled'))),('ASSERT_I04_LEIDENALG_BOUNDARY',all(x in t for x in ('leidenalg 0.12.0','pyproject.toml','blob/0.12.0/LICENSE','_c_leiden','libleidenalg','igraph','rejected'))),('ASSERT_I04_EXACT_URLS',all(all(u.startswith('https://') for u in e['sources']) for e in d.get('entries',[]))),('ASSERT_I04_NO_LEGAL_CONCLUSION','no legal advice, approval, compatibility' in d.get('claim_ceiling',''))]
