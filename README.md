...
`lsp-trace-mcp` is a local-development-only stdio MCP server. Configure an MCP client to start the executable directly; it accepts MCP JSON-RPC on stdin and writes MCP JSON-RPC to stdout. Its current default surface has exactly 23 canonical tools. The authoritative sorted list is: `lsp_session_v1_list`, `lsp_session_v1_restart`, `lsp_session_v1_status`, `lsp_session_v1_stop`, `lsp_trace_v1_bounded_retained_analysis`, `lsp_trace_v1_bounded_retained_metrics`, `lsp_trace_v1_bounded_retained_ranking`, `lsp_trace_v1_capabilities`, `lsp_trace_v1_execute`, `lsp_trace_v1_export_retained_calls`, `lsp_trace_v1_filter`, `lsp_trace_v1_incoming`, `lsp_trace_v1_inspect`, `lsp_trace_v1_inspect_hydrated`, `lsp_trace_v1_schema_get`, `lsp_trace_v1_slice`, `lsp_trace_v1_validate`, `lsp_trace_v1_verify`, `lsp_trace_v2_export_retained_calls`, `lsp_trace_v2_incoming`, `lsp_trace_v2_slice`, `lsp_trace_v2_verify`, and `lsp_trace_v2_verify_retained_calls`. Launch it without filesystem publication, or pin selector publication beneath one private root:
