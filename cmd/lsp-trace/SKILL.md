...
The embedded Stage 1 manifest and registered schemas are authoritative. Each completed tool call returns exactly one versioned envelope in MCP `structuredContent`, selected by `envelope_schema_id`; outer MCP `content` is empty and does not mirror the envelope. Artifact output is inline through 1,048,576 bytes. Larger possible output requires `output_selector`; publication is exclusive, owner-only, no-replace, and returns a path-free receipt. The server never chooses or returns a private path. For potentially broad traversal or Graph V5 sibling expansion, configure `--publication-root` and supply a selector before acquisition. Direct tools take `output_selector` in their arguments. Hidden operations invoked through `lsp_trace_v1_execute` take the graph selector at `request.arguments.output_selector`; the execute gateway's outer selector applies only to the execution artifact. A selector-free oversized result reports its observed size and limit, and retrying reacquires the operation. `list_page_max` is 100.
