...
`context-churn-symbols` is the opt-in CLI-only V2 refinement. It materializes both exact revisions in disposable detached worktrees, starts one caller-selected language server per revision, requests standard LSP document symbols for the artifact's deduplicated paths, and attributes each zero-context changed line to the unique narrowest enclosing server-returned range:
...
Its schema is `lsp-trace.vcs-symbol-churn-sidecar.v2` (`lsp-trace schema get --family vcs-symbol-churn-sidecar --version v2`). MCP operation 39, `lsp_trace_v1_context_symbol_churn`, exposes the same builder using a host-configured language-server profile; `profile` may name either the bootstrap alias (for example `project`) or its configured server profile (for example `gopls`), and unavailable-profile errors list only those non-sensitive configured names in sorted order. Callers cannot provide executable paths, arguments, environments, working directories, publication selectors, or retained inputs. Its success envelope includes a deterministic `summary` with line accounting, integer attribution basis points, and aggregate `COMPLETE`/`EMPTY`/`FAILED` file outcomes while retaining the complete canonical result. A successful result with `line_count: 0` means none of the admitted paths changed in the requested interval; it is distinct from Git, worktree, or LSP acquisition failure. Every changed-line observation is `ATTRIBUTED`, `AMBIGUOUS`, or `UNMATCHED`; every requested file has a `COMPLETE`, `EMPTY`, or `FAILED` acquisition row on each revision. The result retains `authority=0`, `source_graph_complete=UNKNOWN`, and `cross_revision_identity=NOT_EVALUATED`. Names and ranges do not establish symbol continuity, renames, ownership, defects, features, runtime behavior, or `CALLS`. V1 and MCP operation 38 remain unchanged.

MCP operation 40, `lsp_trace_v1_context_symbol_churn_capture`, is the preferred lossless Structural Context→symbol-churn path. It first acquires bounded CALLS-only Structural Context V2 from an exact host-managed session generation, then passes those exact internally serialized bytes to the operation-39 host-profile symbol-churn builder. Its input exposes the structural locator and bounded limits/timeouts plus revisions, profile, and language ID; callers cannot submit structural JSON, workspace paths, commands, arguments, or environments. Structural acquisition, workspace resolution, profile selection, historical symbol acquisition, and delivery validation fail through closed phase-specific states. Direct and canonical-gateway routes preserve the same delegated typed diagnosis. Client schema rejection occurs before dispatch; session/target/profile checks are preflight; call-hierarchy work is traversal; historical acquisition is symbol-churn acquisition; result validation is delivery check; envelope validation is the final transport guard. Operations 36, 38, and 39 remain unchanged.
