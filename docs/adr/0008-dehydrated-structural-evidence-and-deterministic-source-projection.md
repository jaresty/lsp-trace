...
- an optional **structural display range**: a broader presentation range reported by the same managed language server through `textDocument/documentSymbol`, resolved against already-admitted source bytes without changing graph evidence.

A structural display range must contain or otherwise exactly bind its evidence range under the display-resolution contract. Its provenance and method are explicit, currently `SERVER_REPORTED_DOCUMENT_SYMBOL` and `textDocument/documentSymbol`; it cannot add, repair, infer, merge, or strengthen nodes or relations. If no unique server-reported symbol range or admitted bytes exist, display expansion fails closed with a typed terminal disposition rather than presenting an identifier fragment as a definition.
...
Structural display resolution runs only after document admission. It performs one independently bounded `textDocument/documentSymbol` request per selected document against the exact managed session generation. Endpoint definitions match exact symbol selection ranges; relation occurrences select the smallest unique containing server-reported symbol range. Missing or ambiguous matches fail closed. No language-specific parser, import resolution, direct path read, or exact-evidence fallback may manufacture a definition display range. Relation call-site evidence remains the server-reported `fromRange`; caller definition display is a separate presentation range.
