...
func TestProgramCLeidenRegistrySchemasProfilesAndRequiredTopK(t *testing.T) {
	...
	if !ok || len(full.Tools()) != 41 || len(full.Advertised()) != 41 || len(compact.Tools()) != 41 || len(compact.Advertised()) != 12 {
		t.Fatalf("ASSERT_PROGRAM_C_LEIDEN_32_FULL_11_COMPACT: ok=%t full=%d/%d compact=%d/%d", ok, len(full.Tools()), len(full.Advertised()), len(compact.Tools()), len(compact.Advertised()))
	}
