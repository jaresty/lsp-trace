...
Full/race/vet/build/CI/release commands and exact run outcomes belong to the accompanying implementation claim. The pre-existing dirty-worktree ownership guard is not widened for these new paths; commit the isolated change before its clean-tree run. Repository-root/symlink guards are likewise not weakened. This document is not independent-review approval.
