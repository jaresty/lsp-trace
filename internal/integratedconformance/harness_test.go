	...
	t.Run("ASSERT_PACKAGE_OWNERSHIP_ONLY", func(t *testing.T) {
		...
		for _, line := range strings.Split(strings.TrimSpace(string(out)), "\n") {
			...
			owned := ownsFR20FR21Path(path) || path == "sessionruntime/sessionruntime.go" || path == "sessionruntime/sessionruntime_test.go" || path == "sessionruntime/roundtrip_test.go" || path == "sessionruntime/local_darwin_integration_test.go" ||
				...
				path == "internal/graph/relations.go" || path == "internal/graph/relations_test.go" || path == "internal/graph/result.go" || path == "internal/graph/v3.go" || path == "internal/graph/v5_multiple_sibling_test.go" || path == "internal/graph/snapshot_identity.go" || path == "internal/graph/snapshot_identity_test.go" || path == "internal/graph/schemas/normalized-relations.v1.schema.json" ||
