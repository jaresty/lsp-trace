...
func TestTraversalSchemaSelectorContracts(t *testing.T) {
	...
	if _, ok := properties["symbol"]; !ok || tool.InputSchema["oneOf"] == nil || len(NewRegistry(false).Advertised()) != 32 {
...
func TestVerifyAdvertisesGraphProvenanceV5WithoutChangingCardinality(t *testing.T) {
	...
	if got := len(registry.Tools()); got != 32 {
		t.Fatalf("ASSERT_VERIFY_V5_CARDINALITY: got=%d want=32", got)
	}
