...
`LSP_TRACE_FR20_GOPLS=/installed/path/gopls go test ./cmd/lsp-trace-mcp -run '^TestFR20InstalledGopls$' -count=1 -v` runs a disposable multi-file qualification in both modes. Installed gopls v0.23.0 passed native named selectors plus a positional alias, connected/disconnected paths, exact CLI/MCP bytes, four-file capture and source-gone validation/verification. Its flat SymbolInformation response is normalized from `location.range`, filtered by the requested document URI and marked flat in the retained normalized response so replay does not pretend the server supplied a DocumentSymbol selection range. No new server support beyond gopls v0.23.0, dependency coverage, concurrent multi-process qualification, native full-volume or real-product qualification follows from this result.
