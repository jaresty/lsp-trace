...
`./scripts/release-check.sh` is the hermetic release dry-run. It validates required documentation, fixtures, normalized retained PASS evidence for claimed providers, GoReleaser v2 configuration, and the embedded Stage 1 MCP manifest/schema contract. It directly requires production bootstrap coverage, exactly 25 current runtime canonical tools (with the thirteen-tool historical manifest unchanged), the trusted-local warning on stderr and protocol-clean MCP stdout, and then runs the broader focused MCP contract suite. It then uses `go build -trimpath` to require nonempty `lsp-trace` and `lsp-trace-mcp` binaries in a temporary directory. It also runs `./scripts/check-installed-pi-mcp-adapter-canary.sh --required`: release fails closed unless the installed, byte-unmodified `pi-mcp-adapter` v2.32.1 path registers and invokes all three retained D01 analytics canaries with exact payload/digest parity against direct stdio-client calls. Only standalone unit use may skip an unavailable adapter, and only with both `--unit` and `LSP_TRACE_ALLOW_MISSING_PI_MCP_ADAPTER=1`; release never uses that path. The dry-run does not publish, create a tag, modify retained evidence, or invoke GoReleaser. It does not start or install external language servers. CI runs this dry-run plus formatting, Python/shell syntax, `go test ./...`, `go vet ./...`, `go build ./...`, and clean-tree checks.
