...
func newRegistryWithRoutingAndProfile(publicationSupported bool, routing Routing, profile ToolProfile) *Registry {
	...
	descriptions := map[string]string{
		...
		mcpcontract.StructuralContextV2Tool:       "Analyze code structure, architecture, design dependencies, and impact analysis through one exact symbol or exact position target; use an exact position when the declaration URI is known, and otherwise pass the exact server-reported symbol name rather than a package-qualified identifier. Symbol lookup is locator-only before delegation to the shared bounded transient live CALLS-only core. Omit projection for relationship-only context. For source-bearing implementation or ownership analysis, set projection.mode=PROJECTED, projection.body=INCLUDE, projection.display_range_policy=FULL_DEFINITION, and bounded projection limits; this returns exact evidence ranges plus complete server-reported document-symbol definitions without adding graph facts. Authority remains zero, source_graph_complete remains UNKNOWN, and locator evidence never manufactures CALLS",
