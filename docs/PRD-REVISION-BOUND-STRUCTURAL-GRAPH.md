...
5a. **Atomic seed binding (additive v2 boundary):** an explicit `lsp-trace.seed-binding.v2` manifest may bind one seed ID to a canonical workspace-contained file URI, zero-based line/character and declared UTF encoding, expected symbol and declaring file, expected declaration range, source revision, exact SHA-256, and the selected managed LSP identity/version. Its schema is closed; omitted binding preserves historical manifests, defaults, bytes, and errors. Before a provider attempt, host-owned repository custody authenticates the revision; mechanical validation canonicalizes the URI/file, rejects escapes/non-regular/oversized sources, reads at most the bound plus one sentinel byte, retains one exact descriptor snapshot, verifies digest, and validates locator/range ordering, bounds, and exact UTF code-unit boundaries. It permits no checkout, network, provider read, or second source read.
5b. **Same-generation semantic terminal contract:** after mechanical preflight, start and initialize exactly one selected provider generation, send `initialized`, supply the exact retained bytes with `textDocument/didOpen`, then issue one bounded `textDocument/documentSymbol` request on that generation before target resolution or traversal. Strictly decode the array-level `DocumentSymbol[] | SymbolInformation[]` union and reject mixed forms. Hierarchical matches require recursive exact name, selection/name-range containment, declaration-range containment/equality, and the request's canonical source; flat matches require exact name and same canonical location URI/range and record that no distinct name range exists. Exactly one match latches admission; no name-only fallback exists. Only MATCH permits `prepareCallHierarchy`; MISMATCH, UNAVAILABLE, or INVALID issues no call-hierarchy/traversal request and cannot claim zero provider attempts. The semantic request consumes one deterministically ordered request from the shared acquisition request/time limits. No second semantic process, helper, CLI selector pair, or MCP validator bootstrap exists.
5c. **Explicit local caller-asserted custody (additive v3 boundary):** `lsp-trace.seed-binding.v3` adds an explicit `custody_mode` enum. `VERIFIED_HOST` preserves the signed host-receipt path. `CALLER_ASSERTED_LOCAL` is CLI bootstrap input only and requires an exact nonempty revision, canonical workspace-contained file URI/path, lowercase source SHA-256, complete provider identity pins, expected symbol/file/range, and a canonical prepared-manifest SHA-256 when supplied. It reuses the same rooted read-once byte/digest/coordinate validation and the same initialized-generation `didOpen` plus exact `documentSymbol` terminal before target resolution. Local results and diagnostics are labeled `CALLER_ASSERTED`; digest agreement never promotes them to `VERIFIED` or `AUTHENTICATED`. Host verification failure remains terminal and never falls back to local mode. Local execution authority permits bounded execution; it does not establish external provenance authority. MCP request schemas do not expose this mode.
...
Current FR22 matrix row: exact canonical `lsp-trace.normative-retained-graph.v1` bytes → explicit bounded-retained analysis/metrics/ranking v2 CLI and MCP routes → unchanged package-owned local-v1 result bytes. Public production/consumption state is source-ready and unshipped; observed deployed availability is unknown. Qualification is synthetic and local only. The route rejects other graph schemas without an explicit provenance-bearing adapter, and it grants neither permission, Program B admission, nor production authority. Four public family aliases map version `v2` to the unchanged package schema bytes; shared schema retrieval, validation, immutable publication, and offline verification apply without widening frozen verifier semantics.
...
The V3 CLI may explicitly opt in to a private request-failure forensic artifact. This path is additive and must not alter public V1/V2/V3 bytes, MCP tools, schemas, or compositions. It selects one retained failed operation and projects only allowlisted lifecycle facts: opaque manager attempt/session/generation identity; an allowlisted method; dispatch, write, read, response/error, deadline, elapsed, process, ordering, omission, truncation, limit, digest, and length observations. `TIMEOUT_OBSERVED` requires an authoritative deadline terminal; timing near a configured limit is only `FAILED_AT_LIMIT_TIMING_CONSISTENT`, otherwise `FAILED_UNKNOWN`. The artifact must not infer provider causality or contain raw stderr, errors, RPC IDs, messages, params, results, source, URI, path, cwd, environment values, command, or arguments.

Private publication is synchronous but secondary and non-overriding, uses an explicit absolute CLI-selected approved root, pinned-root create-only atomic publication, mode 0600, semantic validation before publication, and digest/length accounting. Validation is closed against unknown fields and known-field smuggling. No opt-in produces no artifact; publication failure emits only a generic public diagnostic and cannot replace public acquisition status or bytes.

A future privately versioned extension may retain digest-only initialize capability/response facts; initialized/didOpen order and timestamps; didOpen version, canonical URI digest, language ID, text length and digest; method-to-wire identity mapping; framing and write/flush/read/dispatch timestamps; matched/unmatched/late disposition; environment key names and value digests; executable-byte and ordered-argument digests; and cwd/workspace-URI digests. Such fields must be captured authoritatively at their owning lifecycle boundaries, never reconstructed or inferred from outcomes, and remain outside public schemas and MCP composition.
...
- For an explicit FR20 seed-binding v2 manifest, complete mechanical source-custody validation before allocating a startup attempt; then perform retained-byte document supply and bounded semantic admission on the same initialized generation before any target resolution. Mechanical failures retain zero attempts. Semantic MISMATCH/UNAVAILABLE/INVALID retain the started attempt and one shared-budget document-symbol request but issue no call-hierarchy/traversal requests. CLI and MCP project the same shared outcome without a validator helper or registry operation.
