...
Choose exactly one starting mode. Legacy `--from-file PATH` is an all-symbol census. Production V5 automatic discovery accepts repeatable `--from-file PATH` file or directory scopes plus repeatable workspace-relative gitignore-style `--include PATTERN` and `--exclude PATTERN`; no includes means all enumerated regular files, and excludes take precedence. Every returned document symbol is a candidate regardless of language or symbol kind, and a candidate is callable only when its exact `selectionRange.start` `textDocument/prepareCallHierarchy` request succeeds with a non-empty result. The canonical path order, prepared-seed order/IDs, 64-seed fail-closed limit, server-only CALLS evidence, and retained canonical `lsp-trace.seeds.v2` custody remain unchanged. Production discovery writes bounded path/name-free exact operational counts for enumerated/selected/excluded files and enumerated/selected/excluded/unsupported/preparation-failed/prepared symbols to stderr; these counts do not claim endpoint or source completeness. Symbols in files excluded before LSP requests are not enumerated, so `symbols_excluded` is exactly zero. Managed `--at PATH:LINE:COLUMN` and managed `--from-file PATH --symbol NAME` are exact target selectors. Outside managed mode, repeatable `--at PATH:LINE:COLUMN` values receive automatic `seed-1`, `seed-2`, … labels, and `--seed-file FILE` uses the existing labeled seed JSON format.
