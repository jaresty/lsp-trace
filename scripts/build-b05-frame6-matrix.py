...
for relation,items in relations.items():
 for path,polarity,outcome in items:
  ...
  for op in ('incoming','slice'):
   ...
   if outcome=='PASS':
    exact={'PASSES_CALLBACK':('PASSES_CALLBACK','callable:format','parameter:callback'), 'UPDATES_STATE':('UPDATES_STATE','state-producer:CounterPanel','state-value:count'), 'RENDERS_FROM':('RENDERS_FROM','property:itemCount','render-expression:this.itemCount')}[relation]
