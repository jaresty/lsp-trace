...
`captureset.Publisher` writes the canonical bundle to one retained source descriptor, fsyncs, seeks and rereads through that same handle, and performs canonical bundle, manifest/constituent bijection, digest, V5 admission, and ceiling verification before publication. It never reopens or trusts a source pathname.

Linux creates the source as an unnamed `O_TMPFILE` in the pinned destination directory and installs that exact descriptor with `linkat(AT_EMPTY_PATH)`. Darwin creates and immediately unlinks a private source, then clones that exact descriptor with `fclonefileat(..., CLONE_NOOWNERCOPY)`. Both kernel operations create the final descriptor-relative name atomically and no-replace. Unsupported filesystems and all other platforms fail closed without a final selector; there is no pathname fallback.

The final-name creation is the namespace commit point. An existing final bundle returns `TARGET_EXISTS` and is never changed. Concurrent same-selector publication has exactly one winner. After commit, the implementation opens the final descriptor-relative object and verifies regular-file identity, owner-only mode, length, exact canonical bytes, digest, native V5 admission, and manifest bijection. A late verification problem is represented as committed success with verification status, never as a failure that suggests visibility was rolled back. Directory sync is closed as `NOT_ATTEMPTED_POST_COMMIT`, `FINAL_DIRECTORY_SYNCED_NO_CRASH_GUARANTEE`, or `COMMITTED_DIRECTORY_SYNC_FAILED`; it is not a whole-filesystem, device, or crash-recovery guarantee. Close status is recorded independently as `COMMITTED_CLOSE_COMPLETE` or `COMMITTED_CLOSE_FAILED` after namespace commit, with `NOT_ATTEMPTED` used only before a close is genuinely attempted. Once committed, directory-sync, close, and verification degradation return a success receipt with the final selector preserved and no retryable publication error.

Resolution accepts only canonical bundle selectors, reads bounded exact bytes beneath the pinned root, and requires an independently supplied native V5 authority to readmit every constituent. It strictly validates manifest/constituent bijection and recomputes logical identity; arbitrary filesystem-created self-consistent bundles do not gain custody. It never derives a constituent from sibling filesystem child names. This is integrity and local custody verification, not authentication or source truth.

The returned `PublicationReceipt` is `PRIVATE` by default. It records selector, exact bundle SHA-256, bundle byte length, constituent count, namespace atomicity, closed directory-sync status, independent close status, committed verification status, and `exact_fd_atomic_no_replace`; it is not a public receipt schema and conveys no additional authority.
