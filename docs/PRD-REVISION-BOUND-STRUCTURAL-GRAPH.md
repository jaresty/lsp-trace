...
5a. **Atomic seed binding before provider launch (additive v2 boundary):** an explicit `lsp-trace.seed-binding.v2` manifest may bind one seed ID to a canonical workspace-contained file URI, zero-based line/character and declared UTF encoding, expected symbol and declaring file, expected declaration range, source revision, exact SHA-256, and authority-labeled language-validator identity/version. Its schema is closed; omitted binding preserves historical manifests, defaults, bytes, and errors. A host-owned revision authority—not a caller string—must approve the pin. Mechanical validation resolves workspace and source symlinks, rejects escapes and non-regular files, checks coordinate bounds and digest over one retained descriptor snapshot, and permits no checkout, network, provider read, or second source read. Semantic validation is performed only through an explicit language-aware `SeedBindingValidator`; core never infers a generic containing function. Results are exactly `MATCH`, `MISMATCH`, `UNAVAILABLE`, or `INVALID`. C# remains `UNAVAILABLE` until an exact offline declaration adapter is supplied; synthetic authority-labeled fixtures do not qualify product bytes.
5b. **Pre-launch terminal contract:** every non-`MATCH` result prevents process creation and provider requests and consumes zero startup attempts or acquisition budgets. Public CLI/MCP outcomes expose only `SEED_LOCATOR_INVALID`, `SEED_SOURCE_MISMATCH`, `SEED_BINDING_MISMATCH`, or `SEED_BINDING_UNAVAILABLE`; private structured detail may retain the cause but public output contains no raw absolute path or source text. `UNAVAILABLE` must not be described as an observed declaration mismatch.
...
- For an explicit FR20 seed-binding v2 manifest, complete both mechanical source-custody validation and authority-labeled semantic binding before allocating a startup attempt or invoking the provider starter. Preserve the four privacy-safe seed terminal codes and zero attempts/budgets on every non-match; CLI and MCP adapters must project the same shared validation outcome without adding a registry operation.
