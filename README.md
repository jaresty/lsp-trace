...
Choose exactly one starting mode. Legacy `--from-file PATH` is an all-symbol census: it enumerates every server-preparable document symbol in one file. Managed `--at PATH:LINE:COLUMN` and managed `--from-file PATH --symbol NAME` are exact target selectors: each identifies one position or symbol. Outside managed mode, repeatable `--at` values receive automatic `seed-1`, `seed-2`, … labels, and `--seed-file FILE` uses the existing labeled seed JSON format. Explicit starts are prepared once, deduplicated by native node identity for traversal, and retain each caller-supplied seed occurrence in `invocation.seeds` and `seeds`.
