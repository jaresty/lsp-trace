...
| Cancellation ambiguity | protocol-visible cancellation, closed accounting, bounded output, descendant termination and cleanup | race cancellation with each operation, timeout, crash and shutdown; reject late or duplicate closure |
...
| Protocol confusion/injection | strict NDJSON direction, bounded frames, versions/types and state transitions | malformed JSON, duplicate keys, oversized/deep, unknown version/type, request before `HELLO`, duplicate `HELLO`, invalid transition and post-`SHUTDOWN` records |
