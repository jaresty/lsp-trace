...
func TestCompactResponsePublishesFullArtifactWithUsabilityMetadata(t *testing.T) {
	...
	if got := len(server.Registry.Advertised()); got != 36 {
...
func TestTraversalCompactResponsePublishesFullArtifact(t *testing.T) {
	const (
		...
		cardinality = "ASSERT_TRAVERSAL_COMPACT_PRESERVES_THIRTY_SIX_TOOLS"
	)
...
func TestRealTraversalEnvelopesValidateAcrossSuccessFailureAndPublication(t *testing.T) {
	const (
		...
		cardinalityAssertion = "ASSERT_REAL_TRAVERSAL_CURRENT36_PRESERVES_HISTORICAL_TOOLS"
	)
...
func TestCapabilitiesDispatch(t *testing.T) {
	const capabilityAssertion = "capabilities returns immutable metadata for all 36 current canonical tools"
...
func TestTransportContract(t *testing.T) {
	...
	const listAssertion = "tools/list advertises the 36 current canonical names"
