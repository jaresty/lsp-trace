...
Provide an explicitly requested, deterministic offline export of reviewable source spans for caller-selected retained nodes, relationships, and declarations. Reuse admitted retained source bytes and provenance rather than reacquiring source. This requirement adds context packaging, not feature-boundary selection, semantic relevance judgments, adjudication, or automatic discovery of containing language/framework constructs.
...
- Diagnostic collection must respect cancellation and resource limits, preserve managed transport ownership and safe teardown, and never block indefinitely to obtain exit or stderr details. Retain the primary failure if diagnostic collection itself fails.
- **Initial delivery scope:** retain safe structured phase/reason codes, observed timing, limits, and exit facts, with raw server stderr, raw errors/RPC messages, environment values, and arguments withheld. Report withheld diagnostic content explicitly; bounded capture alone does not make content safe to release. Rich stderr release/redaction requires a separately approved host policy and is deferred.
- **Delivery boundaries:** provide an opt-in restricted CLI diagnostic file and equivalent host-configured MCP bootstrap sink. Bootstrap may fail before MCP serves requests; do not promise a tool response in that case. Live MCP access uses an explicit versioned structured surface bound to the exact session/generation, with bounded retention and explicit unavailable/evicted outcomes, rather than changing historical error envelopes silently. Diagnostic destinations must reject stdout, unsafe paths/symlinks, and collisions with evidence output; failed delivery must preserve the primary failure. No default diagnostic file writes.
...
- **Failure diagnostics:** local servers exit before initialize, never respond, return protocol errors, and initialize without call-hierarchy support; spawn and document-supply failures also receive distinct phase/reason records. Unknown causes and unavailable observations remain explicit. CLI/MCP classifications agree, and valid empty/unresolved-seed outcomes are not relabeled initialization failures.
...
- FR20–FR23 manifest parity/conflict rejection, stable snapshot paging, cross-page coverage and budgets, executable versioned compatibility-matrix transitions, legacy-null/normalized-array empty results, bounded privacy-safe phase/reason diagnostics, and native/framework coverage-reporting audit fixtures under AC18;
