...
func contextChurnDomainErrorEnvelope(requestID, phase, state, message string) envelope {
...
func contextSymbolChurnDomainErrorEnvelope(requestID, phase, state, message string) envelope {
	return contextSymbolChurnDomainErrorEnvelopeFor(mcpcontract.ContextSymbolChurnTool, mcpcontract.ContextSymbolChurnDomainErrorID, requestID, phase, state, message)
}

func contextSymbolChurnDomainErrorEnvelopeFor(tool, schemaID, requestID, phase, state, message string) envelope {
	return envelope{EnvelopeVersion: "1", EnvelopeSchemaID: schemaID, Tool: tool, RequestID: requestID, Outcome: "DOMAIN_ERROR", OperationStatus: "FAILED", IsError: true, Phase: phase, State: state, Error: map[string]any{"code": state, "message": message}}
}
...
func (s *Server) callContext(ctx context.Context, base response, raw json.RawMessage) response {
	...
	if tool.ExecutorFamily == ContextChurnExecutorFamily {
		var result map[string]any
		if len(opResult.Artifact) == 0 || json.Unmarshal(opResult.Artifact, &result) != nil || mcpcontract.ValidateJSON(mcpcontract.ContextChurnResultID, opResult.Artifact) != nil {
			return bindEnvelope(base, tool, contextChurnDomainErrorEnvelope(requestID, "DELIVERY_CHECK", "CANCELLED", "result validation failed"))
		}
		return bindEnvelope(base, tool, envelope{EnvelopeVersion: "1", EnvelopeSchemaID: mcpcontract.ContextChurnSuccessID, Tool: tool.Name, RequestID: requestID, Outcome: "COMPLETE", OperationStatus: "SUCCEEDED", IsError: false, Result: result})
	}
	if tool.ExecutorFamily == StructuralDeltaExecutorFamily {
		var result map[string]any
...
func validateEmittedEnvelope(tool Tool, env envelope, raw []byte) error {
	...
	case EnvelopePolicyContextSymbolChurnCapture:
		return mcpcontract.ValidateContextSymbolChurnCaptureEnvelopeExclusive(raw)
	default:
...
func operationName(canonical string) operation.Name {
	...
	case mcpcontract.ContextSymbolChurnCaptureTool:
