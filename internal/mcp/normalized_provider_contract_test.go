...
func TestNormalizedProviderCapabilitiesAndPublication(t *testing.T) {
	...
	if len(tools) != 29 || len(registry.Advertised()) != 29 {
		t.Fatalf("ASSERT_MCP_CURRENT29_WITH_HISTORICAL25_PRESERVED: tools=%d advertised=%d", len(tools), len(registry.Advertised()))
	}
