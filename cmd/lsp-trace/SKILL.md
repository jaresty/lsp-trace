...
Configure an MCP client to launch `lsp-trace-mcp --tool-profile compact` directly over stdio for local development. The compact MCP tool-advertisement profile advertises ten core tools while hidden canonical tools and aliases remain directly callable from cached registrations and reachable through `lsp_trace_v1_execute`. Use `--tool-profile full` for the compatibility 32-tool advertisement; full changes discovery only and does not add caller-controlled create or start operations. An MCP tool-advertisement profile controls only `tools/list`; it is unrelated to the CLI `--profile NAME` server launch profile and changes neither dispatch nor managed-session behavior. Run `lsp-trace-mcp --tool-profile compact` for inline-only results, or `lsp-trace-mcp --tool-profile compact --publication-root /absolute/private/root` to permit caller-supplied relative `output_selector` publication beneath one pinned private root. To make traversal callable, the host—not the MCP caller—provisions trusted sessions with `lsp-trace-mcp --bootstrap-config /absolute/path/bootstrap.json`; stdio serving begins only after every configured session reaches correlated READY. Run `lsp-trace-mcp --print-bootstrap-example` to print the exact safe template, including ordered `execution.arguments`, without starting a process. Omit `execution.environment` unless independently required; private diagnostics and retained prestart freezes are not configuration authority. Use `lsp_session_v1_list` to obtain the exact READY session ID and generation.

MCP `line` and `character` values and all LSP/retained ranges are zero-based. CLI `--at PATH:LINE:COLUMN` values are one-based and are converted before the request. Character offsets count code units in the managed session's server-negotiated position encoding (`utf-8` bytes, `utf-16` code units, or `utf-32` code points), never visual columns. The server's initialize capability owns the encoding; omission defaults to `utf-16`, and MCP callers cannot override the generation-bound value.

The compatibility default full surface advertises exactly 32 canonical tools, sorted as: `lsp_session_v1_list`, `lsp_session_v1_restart`, `lsp_session_v1_status`, `lsp_session_v1_stop`, `lsp_trace_v1_bounded_retained_analysis`, `lsp_trace_v1_bounded_retained_metrics`, `lsp_trace_v1_bounded_retained_ranking`, `lsp_trace_v1_capabilities`, `lsp_trace_v1_custody_execute`, `lsp_trace_v1_execute`, `lsp_trace_v1_export_retained_calls`, `lsp_trace_v1_filter`, `lsp_trace_v1_incoming`, `lsp_trace_v1_inspect`, `lsp_trace_v1_inspect_hydrated`, `lsp_trace_v1_program_c_compose`, `lsp_trace_v1_program_c_instability`, `lsp_trace_v1_program_c_leiden`, `lsp_trace_v1_schema_get`, `lsp_trace_v1_slice`, `lsp_trace_v1_validate`, `lsp_trace_v1_verify`, `lsp_trace_v2_bounded_retained_analysis`, `lsp_trace_v2_bounded_retained_metrics`, `lsp_trace_v2_bounded_retained_ranking`, `lsp_trace_v2_export_retained_calls`, `lsp_trace_v2_incoming`, `lsp_trace_v2_slice`, `lsp_trace_v2_verify`, `lsp_trace_v2_verify_retained_calls`, `lsp_trace_v3_incoming`, and `lsp_trace_v3_slice`. Aliases remain callable but unadvertised. Call capabilities with `{}` before relying on schema identities, publication support, or limits. Because MCP `tools/list` describes advertised tools only, call capabilities with `{"operation":"lsp_trace_v3_slice"}` (or another canonical name or alias) to retrieve static registry-owned input, envelope, artifact, advertisement, dispatch, and gateway metadata for a hidden operation. This does not inspect or create a session.

Canonical execute input is exactly `{"request":{"operation":"<canonical operation>","arguments":{...}}}`. Legacy `request.tool` and outer execute selectors are rejected. For example, hydrated inspection may be called directly as `{"input":"{\"schema_version\":\"lsp-trace.graph.v3\",\"invocation\":{},\"capabilities\":{},\"targets\":[],\"nodes\":[],\"edges\":[],\"terminals\":[],\"frontier\":[],\"diagnostics\":[],\"summary\":{}}","node_ids":["node-1"],"endpoint_context":true,"include_bodies":false}`, or through execute as `{"request":{"operation":"lsp_trace_v1_inspect_hydrated","arguments":{"input":"{\"schema_version\":\"lsp-trace.graph.v3\",\"invocation\":{},\"capabilities\":{},\"targets\":[],\"nodes\":[],\"edges\":[],\"terminals\":[],\"frontier\":[],\"diagnostics\":[],\"summary\":{}}","node_ids":["node-1"],"endpoint_context":true,"include_bodies":false}}}`.

The embedded Stage 1 manifest and registered schemas are authoritative. Each completed tool call returns exactly one versioned envelope in MCP `structuredContent`, selected by `envelope_schema_id`; outer MCP `content` is empty and does not mirror the envelope. Artifact output is inline through 1,048,576 bytes. Larger possible output requires `output_selector`; publication is exclusive, owner-only, no-replace, and returns a path-free receipt. The server never chooses or returns a private path. Terminology is strict: a **Target selector** identifies a symbol or position; an **Output destination** is a caller-provided publication location; an **Artifact selector** is the product-generated reference to an immutable artifact written there. For potentially broad traversal or Graph V5 sibling expansion, configure `--publication-root` and supply an output destination before acquisition. Direct tools take `output_selector` in their arguments. Hidden operations invoked through `lsp_trace_v1_execute` take the graph output destination at `request.arguments.output_selector`; the execute gateway's outer output destination applies only to the execution artifact. An oversized result without an output destination reports its observed size and limit, and retrying reacquires the operation. `list_page_max` is 100.

For a compact traversal response, set both `detail: "compact"` and a caller-chosen `output_selector`. Incoming and slice then publish the complete immutable graph bytes while returning a path-free receipt and bounded summary. Omitting `detail`, or using `detail: "full"`, preserves the existing inline/default or full publication semantics. Compact transport does not summarize away, rewrite, or upgrade the retained evidence.
...
- `0`: traversal completed relative to the configured server and bounds; continue with retained evidence. Retained advisory range or normalization warnings may coexist with `traversal_complete: true`: completeness means the requested bounded walks finished, not that warnings are absent or that the graph establishes source truth, runtime coverage, or acceptance.
...
Choose exactly one starting mode. Legacy `--from-file PATH` is an all-symbol census that recursively enumerates document symbols. As a precursor to the intent-oriented interface proposed by ADR 0004, the existing Production V5 slice path accepts repeatable file/directory `--from-file` scopes and repeatable `--include` and `--exclude` patterns. Patterns use a workspace-relative gitignore-style glob subset, not full `.gitignore` parsing: `*`, `?`, character classes, path segments, `**`, an optional leading `/` workspace anchor, and a trailing `/` directory form are supported. Empty or malformed patterns and leading `!` negation are rejected before source enumeration or server start; exclusions always win. Every document symbol is a candidate regardless of kind. Only an exact successful, non-empty `prepareCallHierarchy` at `selectionRange.start` makes it callable. Managed `--at PATH:LINE:COLUMN` and `--from-file PATH --symbol NAME` are exact targets. Outside managed mode, repeatable `--at` values use one-based explicit positions with automatic labels; `--seed-file FILE` uses the existing labeled seed format. Explicit seed occurrences retain their labels and requested positions even when prepared nodes deduplicate. ADR 0004's `trace` and `discover` commands remain proposed and are not implemented here.

Slice accepts `--server-arg`, `--server-env`, `--language-id`, `--down-depth`, `--up-depth`, `--max-nodes`, `--timeout`, `--request-timeout`, `--output`, and `--pretty`. Production V5 slice acquisition may additionally use repeatable `--include`/`--exclude` discovery filters and `--group-by leiden --community-seed N --pagerank-top-k N --hub-top-k N`. Automatic discovery writes bounded path/name-free operational accounting to stderr for enumerated/selected/excluded/unsupported/document-supply-failed/document-symbol-failed/processed/incomplete files and enumerated/selected/excluded/unsupported/preparation-failed/prepared/incomplete symbols. The denominators reconcile exactly; a file-local failure does not suppress later safe census work, and an incomplete census fails closed after accounting. It does not claim endpoint or source completeness, and pre-LSP file filtering means `symbols_excluded` is exactly zero. All grouping options and `--output GRAPH_SELECTOR` are required, the immutable exact Graph Provenance V5 graph is published to that selector first, and stdout contains only the grouped text presentation. Omitted or `none` grouping preserves the existing acquisition output. Unknown grouping and non-V5 acquisition are rejected before acquisition. Defaults are down-depth 2, up-depth 100, max-nodes 10000, global timeout 5m, and request timeout 30s. Zero disables traversal in either direction; max-nodes and global timeout use zero for unlimited. Request timeout must be positive. Slice is v3-only and does not accept incoming-only schema, max-depth, provenance, expansion, logging, concurrency, or transcript flags.

The command asks the server which positions prepare as call-hierarchy items and walks server-reported outgoing calls toward the exact `--down-depth` layer. `frontier_node_ids` contains only nodes at that exact depth. `outgoing_terminal_node_ids` contains nodes reached earlier whose `outgoingCalls` request succeeded with an empty result. `upward_start_node_ids` is the native-ID-sorted union of those two sets, and only that union feeds the existing incoming traversal. Every upward start is depth zero for its own walk, so `--up-depth` counts incoming edges independently from each union member. Null, failed, timed-out, canceled, or node-budget-truncated outgoing expansion is not a server-reported leaf. A node-budget cutoff with unresolved outgoing expansion is conservatively incomplete even when the request that did not finish might have returned no calls. Both depths count edges; zero disables traversal in that direction. The v3 `slice` section references native node and relation IDs rather than duplicating graph records.
