	...
	t.Run("ASSERT_PACKAGE_OWNERSHIP_ONLY", func(t *testing.T) {
		...
		for _, line := range strings.Split(strings.TrimSpace(string(out)), "\n") {
			...
			owned := ownsFR20FR21Path(path) || path == "sessionruntime/sessionruntime.go" || path == "sessionruntime/sessionruntime_test.go" || path == "sessionruntime/roundtrip_test.go" || path == "sessionruntime/local_darwin_integration_test.go" ||
				...
				path == "scripts/check-program-c-profiles.py" || path == "scripts/test-program-c-profiles.py" || path == "scripts/qualify-program-c-ember-callback-flow.py" || path == "scripts/test-program-c-ember-callback-flow.py" || path == "qualification/program-c/profile-qualification-receipt.v1.schema.json" || path == "qualification/program-c/profile-qualification.tsv" || path == "qualification/program-c/callback-flow-v1-typescript-ember-ember-glint-1.0.3.receipt.json" || path == "schema/schemas/lsp-trace.program-c-profile-qualification-receipt.v1.schema.json" || path == "internal/schema/schemas/lsp-trace.program-c-profile-qualification-receipt.v1.schema.json"
			if !owned {
