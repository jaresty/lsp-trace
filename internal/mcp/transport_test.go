...
func TestCompactResponsePublishesFullArtifactWithUsabilityMetadata(t *testing.T) {
	...
	if got := len(server.Registry.Advertised()); got != 29 {
...
func TestTraversalCompactResponsePublishesFullArtifact(t *testing.T) {
	const (
		...
		cardinality = "ASSERT_TRAVERSAL_COMPACT_PRESERVES_TWENTY_NINE_TOOLS"
	)
...
func TestRealTraversalEnvelopesValidateAcrossSuccessFailureAndPublication(t *testing.T) {
	const (
		...
		cardinalityAssertion = "ASSERT_REAL_TRAVERSAL_CURRENT29_PRESERVES_HISTORICAL_TOOLS"
	)
...
func TestCapabilitiesDispatch(t *testing.T) {
	const capabilityAssertion = "capabilities returns immutable metadata for all 29 current canonical tools"
...
func TestTransportContract(t *testing.T) {
	...
	const listAssertion = "tools/list advertises the 29 current canonical names"
