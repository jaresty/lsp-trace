...
MCP operation 39, `lsp_trace_v1_context_symbol_churn`, exposes the same builder using a host-configured language-server profile; `profile` may name either the bootstrap alias (for example `project`) or its configured server profile (for example `gopls`), and unavailable-profile errors list only those non-sensitive configured names in sorted order. Callers cannot provide executable paths, arguments, environments, working directories, publication selectors, or retained inputs. Its versioned success envelope includes a deterministic `summary` with line accounting, integer attribution basis points, and aggregate `COMPLETE`/`EMPTY`/`FAILED` file outcomes while retaining the complete canonical result. A successful result with `line_count: 0` means none of the admitted paths changed in the requested interval; it is distinct from Git, worktree, or LSP acquisition failure. Every changed-line observation remains `ATTRIBUTED`, `AMBIGUOUS`, or `UNMATCHED`; ambiguous and unmatched lines remain visible and outside symbol counts. Every requested file has a `COMPLETE`, `EMPTY`, or `FAILED` acquisition row on each revision. The result retains `authority=0`, `source_graph_complete=UNKNOWN`, and `cross_revision_identity=NOT_EVALUATED`. Names, ranges, and density do not establish symbol continuity, ownership, defects, features, runtime behavior, graph authority, or `CALLS`; density cannot manufacture a call relation. V1, V2, and MCP operation 38 remain unchanged.
