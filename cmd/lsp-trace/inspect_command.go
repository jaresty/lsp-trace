...
type inspectAllProjection struct {
	InspectionSchemaVersion string `json:"inspection_schema_version"`
	ProjectionKind          string `json:"projection_kind"`
	Authority               string `json:"authority"`
	ArtifactIdentity        struct {
	...
	Records    inspectRecords    `json:"records"`
	Seeds      []inspectAllSeed  `json:"seeds"`
	Accounting inspectAccounting `json:"accounting"`
}

func validateAllSeedAccounting(projection inspectAllProjection) error {
	expected := inspectAccounting{
...
func runInspect(args []string, stdout, stderr io.Writer) int {
	...
	var projection any
	if *allSeeds {
		projection, err = projectAllSeedInspection(data)
	} else {
...
func projectSeedInspection(data []byte, label string) (inspectProjection, error) {
	...
	out := inspectProjection{InspectionSchemaVersion: schema.InspectionVersionV1, ProjectionKind: "SEED_INSPECTION", Authority: "NON_AUTHORITATIVE_DERIVED_VIEW", Seed: *selected}
...
func projectAllSeedInspection(data []byte) (inspectAllProjection, error) {
	var bundle inspectBundle
	if err := json.Unmarshal(data, &bundle); err != nil {
		return inspectAllProjection{}, err
	}
	out := inspectAllProjection{InspectionSchemaVersion: schema.InspectionVersionV1, ProjectionKind: "ALL_SEEDS", Authority: "NON_AUTHORITATIVE_DERIVED_VIEW"}
	out.ArtifactIdentity.ExecutionBundleID = bundle.ExecutionBundleID
	out.ArtifactIdentity.SemanticCommitmentDigest = bundle.TraceReceipt.SemanticCommitmentDigest
	out.ArtifactIdentity.ExactSerializedBytesDigest = graph.ExactBytesDigest(data)
	out.Records = inspectRecords{
	...
	out.Seeds = make([]inspectAllSeed, 0, len(bundle.Invocation.Seeds))
	for _, requested := range bundle.Invocation.Seeds {
		matches := results[requested.Label]
		if len(matches) != 1 {
			return inspectAllProjection{}, fmt.Errorf("seed label %q has %d results; expected exactly one", requested.Label, len(matches))
		}
		seed := matches[0]
		...
		entry := inspectAllSeed{PreparationStatus: "SUCCEEDED", Seed: seed, SeedMemberships: []graph.SeedMembership{}, NativeNodeIDs: append([]string{}, seed.ReachedNodeIDs...), NativeCallRelationIDs: append([]string{}, seed.ReachedRelationIDs...), DiscoveryNominationIDs: []string{}, CorrelatedDiagnosticIndexes: []int{}}
		if seed.Failure != nil {
	...
	if err := validateAllSeedAccounting(out); err != nil {
		return inspectAllProjection{}, fmt.Errorf("inspection accounting: %w", err)
	}
	return out, nil
}
