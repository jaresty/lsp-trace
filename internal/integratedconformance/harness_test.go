	...
	t.Run("ASSERT_PACKAGE_OWNERSHIP_ONLY", func(t *testing.T) {
		...
		for _, line := range strings.Split(strings.TrimSpace(string(out)), "\n") {
			...
			owned := ownsFR20FR21Path(path) || path == "sessionruntime/sessionruntime.go" || path == "sessionruntime/sessionruntime_test.go" || path == "sessionruntime/roundtrip_test.go" || path == "sessionruntime/local_darwin_integration_test.go" ||
				...
				path == "internal/schema/denominator.go" || path == "internal/schema/denominator_test.go" || path == "internal/schema/family_test.go" || path == "internal/schema/schema.go" || path == "internal/schema/validation_core.go" || path == "internal/schema/compiled_cache.go" || path == "internal/schema/schemas/lsp-trace.source-denominator.v1.schema.json" || path == "internal/schema/schemas/lsp-trace.graph.v4.schema.json" || path == "schema/schemas/lsp-trace.execution.v1.schema.json" ||
