...
func TestCompactResponsePublishesFullArtifactWithUsabilityMetadata(t *testing.T) {
	...
	if got := len(server.Registry.Advertised()); got != 35 {
...
func TestTraversalCompactResponsePublishesFullArtifact(t *testing.T) {
	const (
		...
		cardinality = "ASSERT_TRAVERSAL_COMPACT_PRESERVES_THIRTY_FIVE_TOOLS"
	)
...
func TestRealTraversalEnvelopesValidateAcrossSuccessFailureAndPublication(t *testing.T) {
	const (
		...
		cardinalityAssertion = "ASSERT_REAL_TRAVERSAL_CURRENT35_PRESERVES_HISTORICAL_TOOLS"
	)
	...
	if got := len(registry.Advertised()); got != 35 {
...
func TestCapabilitiesDispatch(t *testing.T) {
	const capabilityAssertion = "capabilities returns immutable metadata for all 35 current canonical tools"
...
func TestTransportContract(t *testing.T) {
	...
	const listAssertion = "tools/list advertises the 35 current canonical names"
